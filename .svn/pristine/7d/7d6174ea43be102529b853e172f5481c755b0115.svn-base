$(function () {
  ...
  $.ajax({
    type:'get',
    url:url+'/getDeviceById',
    data:'username='+username+'&did='+global.id,
    error:function(error){console.log(error)},
    success:function(res){
      console.log(res)
      $('#imei').val(res.imei)
      $('#address').val(res.adss)
      $('#type').val(res.type=='除湿机'?'1':res.type=='电力系统检测'?'2':res.type=='缆温'?'3':res.type=='视频'?'4':res.type=='视频'?'5':'')
      // $('#type').val('2')
      $('#name').val(res.dname)
  ...
  $('#save-btn').click(function (e) {
    ...
    if(imei==''){
      ...
      $.ajax({
        type: 'get',
        url: url + '/editDevice',
        data: 'name=' + name + '&username=' + username + '&adss=' + address + '&remark=' + remark+'&type='+type+'&imei='+imei,
        error: function (error) {
          console.log(error)
        },
        success: function (result) {
